<?php
	...
	if($_POST){
		if ($_POST['action'] == 'addSighting') {
			...
			$query = "INSERT INTO sightings (sighting_date, creature_type,
					creature_distance, creature_weight, creature_height,
					creature_color, sighting_latitude, sighting_longitude) ";
			$query .= "VALUES ('$my_date', '$type', '$distance', '$weight',
				'$height', '$color', '$lat', '$long') ";
	
			$result = db_connection($query);
			
			if ($result) {
				$msg = "Creature added successfully - " + $query;
				success($msg);
			} else {
				fail('Insert failed - ' + $query);
			}
			exit;
		}
	}
	...
	if($_GET) {
		if($_GET['action'] == 'getAllSightings') {
			$query = "SELECT sighting_id, sighting_date, creature_type" +
					" FROM sightings ORDER BY sighting_date ASC";
			$result = db_connection($query);
			
			$sightings = array();
			while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
				array_push($sightings, array('id' => $row['sighting_id'],
